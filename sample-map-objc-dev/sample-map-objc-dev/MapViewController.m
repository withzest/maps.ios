...
- (IBAction)showOverlayLayers:(UIBarButtonItem *)sender {
    ...
    UIAlertController* alertCotrl = [UIAlertController alertControllerWithTitle:@"Overlay Layers"
                                                                   message:nil
                                                                 preferredStyle:UIAlertControllerStyleActionSheet];

    UIAlertAction* action1 = [UIAlertAction actionWithTitle:[NSString stringWithFormat:@"Traffic layer is %@", (self.mapView.mapViewTrafficMode ? @"On" : @"Off")]
                                                      style:UIAlertActionStyleDefault
                                                     handler:^(UIAlertAction * action) {
                                                         self.mapView.mapViewTrafficMode = !self.mapView.mapViewTrafficMode;
                                                     }];
    UIAlertAction* action2 = [UIAlertAction actionWithTitle:[NSString stringWithFormat:@"Bicycle layer is %@", (self.mapView.mapViewBicycleMode ? @"On" : @"Off")]
                                                      style:UIAlertActionStyleDefault
                                                     handler:^(UIAlertAction * action) {
                                                         self.mapView.mapViewBicycleMode = !self.mapView.mapViewBicycleMode;
                                                     }];

    [alertCotrl addAction:action1];
    [alertCotrl addAction:action2];
    [alertCotrl addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil]];
    [self presentViewController:alertCotrl animated:YES completion:nil];
}
